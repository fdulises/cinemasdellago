<?php
	...
	$lista = extras::htmlentities(user::getList(array(
		'columns' => [
			'id', 'nickname', 'email', 'role', 'total_post', 'created'
		],
		'limit' => $per_page,
		'offset' => $offset,
		'order' => 'created ASC',
	...
	foreach($lista as $k => $v){
		...
		$lista[$k]['link'] = site::getInfo('url').'/@'.$v['nickname'];
...
?>
	<div class="container listable">
		<div class="gd-100">
			<div class="gd-60 gd-m-100"><h1><span class="icon-user"></span> Listado de usuarios</h1></div>
			<div class="gd-40 gd-m-100 tx-right">
				<a href="<?php echo APP_ROOT; ?>/usuarios/crear"><button type="button" class="btn btn-primary"><span class="icon-plus"></span></button></a>
			</div>
		</div>
		<div class="container">
		...
		<div class="container">
			...
			<div class="gd-20 gd-m-50 tx-right">
				<a href="usuarios/editar/<?php echo $c['id']; ?>"><button class="btn btn-primary size-s"><span class="icon-pencil"></span></button></a>
			</div>
		</div>
		<?php endforeach; ?>
	</div>
